...
public class PhysicalFighters extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if ((Invincibility) && (EarlyInvincibleTime <= 0)) {
            getLogger().info("초반무적이 1분으로 설정됩니다. [E.시간이 0분 이하입니다]");
            EarlyInvincibleTime = 1;
        }
        if (RestrictionTime < 0) {
            getLogger().info("제약 시간 값은 0보다 커야합니다. 0으로 설정됩니다.");
            RestrictionTime = 0;
        }
        getLogger().info(String.format("능력 %d개가 등록되있습니다.", AbilityList.AbilityList.size() - 1));
        if (Kimiedition) {
            getLogger().info("극한모드 적용");
        }
        if (Specialability) {
            getLogger().info("안좋은 능력을 제거합니다.");
        }
        if (Gods) {
            getLogger().info("신 등급 활성화!");
        }
    }
