...
public class EventManager implements Listener {
    ...
    public static void onEntityDeath(EntityDeathEvent event) {
        if (MainScripter.Scenario == ScriptStatus.GameStart && event instanceof PlayerDeathEvent pde) {
            ...
            // 인벤세이브 처리
            if (PhysicalFighters.InventorySave && !AbilityList.phoenix.PlayerCheck(victim)) {
            ...
            if (killer != null || !PhysicalFighters.AllowND) {
                ...
                // 킥, 밴 처리
                if (PhysicalFighters.AutoKick && !AbilityList.phoenix.PlayerCheck(victim)) {
        ...
        if (PhysicalFighters.AbilityOverLap) {
            if (ability.GetAbilityType() == AbilityBase.Type.Active_Continue ||
                ...
                for (AbilityBase ab : AbilityList.AbilityList) {
                    if (ab.PlayerCheck(p) && (ab.GetAbilityType() == AbilityBase.Type.Active_Continue ||
